...
{
    public class ActivityPrizeFluentValidator : AbstractValidator<ActivityPrize>
    {
        public ActivityPrizeFluentValidator()
        {
        ...
        {
            var result = await ValidateAsync(ValidationContext<ActivityPrize>.CreateWithOptions((ActivityPrize)model, x => x.IncludeProperties(propertyName)));
            if (result.IsValid)
